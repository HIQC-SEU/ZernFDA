...
set_device("gpu")
os.environ["CUDA_VISIBLE_DEVICES"] = "2"	# 需要根据当前可用的GPU ID来定,当有多块GPU时,这里指定后,后续只能看到1块,并且该块的ID识别变为0
...
def test_zernipax(
	...
	# 打印
	print("> INFO: [python] {0}th order, {1}th test time. Start zernipax evaluation on {2} platform!".format(info_list[0],info_list[1],"gpu"))
	...
	# 打印
	print("> INFO: [python] {0}th order, {1}th test time. zernipax-{2} load tensor consumes {3} s".format(info_list[0],info_list[1],"gpu",t_zernipax_load))
	...
	# 打印
	print("> INFO: [python] {0}th order, {1}th test time. zernipax-{2} consumes {3} s".format(info_list[0],info_list[1],"gpu",t_zernipax))
